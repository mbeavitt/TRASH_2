fill_gaps <- function(repeat_table, fasta_content, array_start) {
  ...
  while (i <= nrow(repeat_table)) {
    ...
    if (gap_len %in% one_rep_range) {
      ...
      check_seq_fw <- extract_sequence_string(fasta_content, (new_start - array_start + 1), (new_end - array_start + 1))
      check_seq_rv <- rev_comp_string(check_seq_fw)
      score <- calculate_edit_distance_score(repeat_table$representative[i], c(check_seq_fw, check_seq_rv)) / 100
