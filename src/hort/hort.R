hort <- function(cmd_arguments) {
  ...
  alignment_name <- paste(cmd_arguments$class, "_", cmd_arguments$chrA, runif(1, 0, 1), sep = "_")
  ...
  if (nrow(hors) > 1) {
    ...
    write.csv(x = hors, file = file.path(cmd_arguments$output_folder, paste0("HORs_", cmd_arguments$class, "_", cmd_arguments$chrA, ".csv", collapse = "")))
  ...
  png(filename = file.path(cmd_arguments$output_folder, paste0("HORs_lines_", cmd_arguments$class, "_", cmd_arguments$chrA, ".png", collapse = "")), width = 6000, height = 6000, pointsize = 80)
